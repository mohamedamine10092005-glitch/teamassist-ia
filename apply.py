...
def compute_scores(mood: int, workload: str, sleep: int, focus: int, conflicts: str, comment: str):
    ...
    # Mini “NLP” (démonstrateur) : détection de mots-clés
...
def generate_recommendations(stress: int, motivation: int, risk: int, keyword_hit: bool):
    ...
    if level == "Élevé":
        ...
        team_actions += [
            ...
            "Demander de l’aide sur une tâche précise (binômage / relecture / support).",
        ]
        if keyword_hit:
        ...
        team_actions += [
            "Lister les blocages et formuler un besoin clair (aide / ressources / délai).",
            "Mettre en place une routine courte de suivi (5 min/jour).",
        ]
    else:
        manager_actions += [
            "Maintenir le cadre actuel et valoriser les efforts.",
            "Préserver l’équilibre : charge stable, feedback régulier.",
        ]
        team_actions += [
...
def comment_suggestions():
    ...
    (pour rendre la saisie rapide et guidée).
    """
    return {
        "Charge & délais": [
            ...
            "Les attentes ne sont pas totalement claires, j’aimerais un point rapide.",
        ],
        "Énergie & sommeil": [
            "Je dors mal en ce moment, je manque d’énergie.",
            "Je suis fatigué(e) et j’ai du mal à rester concentré(e).",
            "Je me sens plus lent(e) que d’habitude, j’ai besoin de pauses.",
            "Mon rythme est instable, j’aimerais mieux planifier ma semaine.",
        ],
        "Motivation": [
            ...
            "J’aimerais avoir un feedback pour savoir si je suis sur la bonne voie.",
        ],
        "Relationnel & communication": [
            ...
            "J’aimerais améliorer la coordination pour éviter les blocages.",
        ],
    }
...
with st.sidebar:
    ...
    st.caption("Astuce : garde l’anonymisation activée pour la démo.")
...
with col_dash:
    ...
    if submitted:
        ...
        with st.container(border=True):
            ...
            st.markdown(summary)
            if keyword_hit:
                st.warning("Mots-clés sensibles détectés dans le commentaire. Prioriser un échange humain.")
        ...
        with st.expander("🔎 Explicabilité (comment l’IA a conclu ?)"):
            st.write(
                "- Le score de risque est estimé à partir de : charge, humeur, sommeil, concentration et tensions.\n"
                "- Des mots-clés sensibles peuvent augmenter légèrement le risque pour renforcer la vigilance.\n"
                "- Modèle **démonstrateur pédagogique** : transparent, améliorable et non décisionnel."
            )
...
# Footer discret
st.divider()
st.caption("TeamAssist IA — Prototype pédagogique • IA conseillère, décision humaine • Démonstrateur de management augmenté")
